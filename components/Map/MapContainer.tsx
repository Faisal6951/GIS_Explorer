...
import { applyGisLayers } from '@/lib/mapLayers';
import { gibsTileUrl, RS_LAYER_ID, RS_SOURCE_ID, RS_OVERLAYS } from '@/lib/rsOverlays';
import { haversineDistance, calcPolygonArea, fetchElevationProfile } from '@/lib/utils';
import {
  resolveUserLocation,
  getIpApproxLocation,
  reverseGeocode,
  ...
  return (
    <div className="relative w-full h-full">
      ...
      {state.activeRSIndex && RS_OVERLAYS[state.activeRSIndex] && (
        <div className="absolute top-14 right-3 z-10 bg-purple-500/15 backdrop-blur border border-purple-500/40 text-purple-300 text-[10px] font-mono px-2.5 py-1.5 rounded-md max-w-[200px]">
          {RS_OVERLAYS[state.activeRSIndex].label}
        </div>
      )}

      {state.pickLocationOnMap && (
        <div className="absolute top-[4.5rem] left-1/2 -translate-x-1/2 z-10 bg-purple-500/15 backdrop-blur border border-purple-500/40 text-purple-200 text-[11px] font-mono px-3 py-1.5 rounded-full pointer-events-none whitespace-nowrap">
      ...
      {state.activeTool && !state.pickLocationOnMap && (
        <div className="absolute top-[4.5rem] left-1/2 -translate-x-1/2 bg-surface-1/90 backdrop-blur border border-accent-cyan/30 text-accent-cyan text-[11px] font-mono px-3 py-1.5 rounded-full pointer-events-none animate-fade-in whitespace-nowrap">
          <span className="cursor-blink mr-1">▶</span>
